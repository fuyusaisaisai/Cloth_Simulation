...
public:
    ...
    {
    public:
        Box() : Primitive(BOX), m_min(glm::vec3(0.0f)), xlength(1.0f), ylength(1.0f), zlength(1.0f) {init_visualization();};
        Box(const glm::vec3 pos, float xlen, float ylen, float zlen) : Primitive(BOX), m_min(pos), xlength(xlen), ylength(ylen), zlength(zlen) {init_visualization();};
        Box(const Box& other) : Primitive(other), m_min(other.m_min), xlength(other.xlength), ylength(other.ylength), zlength(other.zlength) {};
        virtual ~Box() {};
